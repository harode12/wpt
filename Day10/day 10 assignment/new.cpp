...
public:
    Rental(Vehicle* v = nullptr, int h = 0) : rentedVehicle(v), hours(h) {}

    double getTotalCost() const {
        return rentedVehicle->getRate() * hours;
    }

    // Simplified operator+ (if needed in future)
    Rental operator+(const Rental& other) {
        return Rental(rentedVehicle, hours + other.hours);
    }

    // Overload << to display rental bill
    friend ostream& operator<<(ostream& os, const Rental& r) {
        os << "\n------ Rental Bill ------\n";
        if (r.rentedVehicle)
            r.rentedVehicle->getDetails();
        os << "Hours Rented: " << r.hours << endl;
        os << "Total Cost: " << r.getTotalCost() << " Rs\n";
        os << "-------------------------\n";
        return os;
    }
};
...
int main() {
    ...
    if (choice >= 1 && choice <= 5) {
        r = Rental(&cars[choice - 1], hours);
    } else if (choice >= 6 && choice <= 10) {
        r = Rental(&bikes[choice - 6], hours);
    } else {
        cout << "Invalid choice!" << endl;
        return 0;
    }

    // Display bill using overloaded <<
    cout << r;
